...
proc mtest {description dtinput mdoutput} {
	global testnum
	test markup-[incr testnum] $description -setup {
		::doctools::new doc -format ../markdown.doctools.tcl
	} -body {
		doc format [dth $dtinput]
	} -cleanup {
		doc destroy
	} -result [mdh $mdoutput]
}
...
mtest "image with label" {[image foo bar]} {![bar](foo)}
...
mtest "sectref w/label"  {[sectref foo bar]} {**bar**}
...
mtest "uri with label"   {[uri foo bar]}   {[bar](foo)}
